"""
Evaluate Multi-Query Retriever with RAGAS

This script loads the shared state and evaluates Multi-Query retrieval
...
os.environ["LANGCHAIN_PROJECT"] = "financial_advisor_multi_query_evaluation"
...
def load_shared_state():
    ...
    vector_store = Qdrant.from_documents(
        split_documents,
        embeddings,
        collection_name="financial_advisor_multi_query_eval"
    )
...
def test_langsmith_tracing():
    """
    Test LangSmith tracing to ensure it's working properly
    """
    print("🧪 **Testing LangSmith Tracing**")
    print("=" * 50)
    
    if not os.environ.get("LANGSMITH_API_KEY"):
        print("⚠️ LangSmith API key not set - skipping tracing test")
        return False
...
def create_rag_chain(retriever):
    ...
    """
    print("🔗 **Creating RAG Chain with Multi-Query Retriever**")
    print("=" * 60)
...
def display_results(result):
    ...
    Display Results
    """
    print("📊 **Multi-Query Retriever Evaluation Results**")
    print("=" * 60)
...
def save_results(dataset, result):
    ...
    if result:
        try:
            ...
            results_filename = f"results/multi_query_results_{timestamp}.json"
            ...
            results_filename = f"results/multi_query_results_{timestamp}.txt"
            with open(results_filename, 'w') as f:
                f.write(str(result))
...
def main():
    """
    Main function - run the complete evaluation
    """
    print("🚀 **Multi-Query Retriever Evaluation**")
    print("=" * 60)
    print()
    
    # Test LangSmith tracing first
    tracing_ok = test_langsmith_tracing()
    if not tracing_ok:
        print("⚠️ LangSmith tracing not working - evaluation will continue without tracing")
    print()
